SD<-function(dat1,dat2,sigqtl.index){
...
##解微分方程，求得SNP互作数据
optim_inter <- function(all_cluster_value,ind_Lpar11,norder,time){
  ...
  ttt <- seq(1,max(t),length.out = (dim(ind_LparAA)[1]+5))
  ...
  tryyAA <- optim.parallel(connect=clusterAA_50$connect,effect=t(all_cluster_value),
                           n.cores=1,proc=ode.optim,order=norder,times=seq(1,max(time),0.5),nstep=length(seq(1,max(time),0.5)-1))
  ...
  for (n in 1:length(clusterAA_list)) {
    ...
    clusterAA_list[[n]][[3]] <- colSums(get_integrate(out=tryyAA[[n]],t=seq(1,max(time),0.5)))
...
out_Netdata_cytoscape <- function(optim_cluster,filename,clusterAA){
  ...
  write.csv(new,filename, row.names = F,fileEncoding = "UTF-8",quote = F)
